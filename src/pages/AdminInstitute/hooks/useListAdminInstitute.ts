...
import {getInstituteList, InstitutesListType} from "@/entities/Institute";
...
export function useListAdminInstitute() {
    const router = useRouter()
    const [data, setData] = useState<InstitutesListType | null | undefined>(undefined)
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            const response = await getInstituteList({skip, limit})
            setData(response)
        }
        getData()
    ...
    const body: TableBodyType | undefined = data?.data?.map(item => {
        return [{type: 'field', label: item.id}, {type: 'field', label: item.name}]
    })

    const onClickRow = (idx: number) => {
        const id = data?.data[idx]?.id
        if (id !== undefined) {
            router.push(ROUTES_CONFIG.ADMIN_INSTITUTE_DETAIL_SLUG + `${id}`)
        }
    }
